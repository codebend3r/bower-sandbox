...
(function () {
  ...
  angular.module('ute.ui.livesite', [
...
function validateForgotPassword(e) {
  ...
  /* $.ajax({ //Process the form using $.ajax()
   ...
   data: {
   'userName': document.getElementById("capture_forgotPassword_resetPasswordUserID").value
   },
   dataType: 'json',
  ...
  if (regex.test(document.getElementById("capture_forgotPassword_resetPasswordUserID").value)) {
    ...
    document.getElementById('janrainModal').style.height = 'auto';
  } else {
    $('#capture_forgotPassword_form_item_resetPasswordUserID .capture_tip_error').html(janrain.globalResult.userIdValidation.required);
    $('#capture_forgotPassword_form_item_resetPasswordUserID .capture_tip_error').show();
  }
}
...
function janrainCaptureWidgetOnLoad() {
  ...
  /* this is bind events with JanRain  into services/uteJanrainEvents.js*/


  if (typeof __karma__ != 'object') {
    var elem = angular.element($('body'));
    var injector = elem.injector();
    var myService = injector.get('uteJanrainEvents');
    console.log(myService);
    myService.checkJanrainUiReady();
...
(function () {
  ...
  angular.module('ute.ui.livesite')
    ...
    .directive('uteHeader', function ($rootScope, $http, $compile, uteEndpoint, uteEnv, commonService, $parse) {

      return {
        restrict: 'AE',
        scope: true,
        replace: false,
        controller: 'uteHeaderCtrl',
  ...
  angular.module('ute.ui.livesite')
    ...
    .controller('uteHeaderCtrl', function ($scope, $rootScope, $state, $window, uteUserService, uteModal, uteCartManager, uteJanrainService) {
      ...
      $scope.openSearch = function () {
        uteModal.open({
          templateUrl: 'views/uteModal/search.html',
    ...
    .factory('uteCartManager', function ($rootScope, $q, $state, $http, uteModal, uteEndpoint, uteEnv, uteCoreFactory) {
      ...
      uteCartManager.internet = {
        ...
        removeOffer: function (tierId, productId, offerId) {
          ...
          var deferred = $q.defer(),
            modalConfig = {
              templateUrl: './scripts/ute-ui-extended/html/remove-from-cart.html',
          ...
          if (uteCartManager.maxItemsInCart('internet')) {
            ...
            var tierInCart = this.tierInCart(),
              modalConfig = {
                ...
                templateUrl: 'modules/checkout/views/add-to-cart.html',
                ok: function () {
            ...
            uteModal.processRequest({
              ...
              //deferred.resolve(data);
              //uteMiniCartCtrl.internet.stockInternetTierToCart(tier.selectedOffer);
              deferred.reject();
            });
      ...
      uteCartManager.maxItemsInCart = function (service) {
        ...
        if (uteCartManager.getQuantity() > 0) {
          console.log('rootScope cart quantity', $rootScope.shoppingCart[collections.userPref.language].services[service].quantity);
